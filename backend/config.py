...
# dev: Vite dev :3000 / :5173, preview :4173 (run-all.cmd); 127.0.0.1 — если открывают не localhost
...
# Dev: после локальной записи не требовать совпадения с GET API (туннель/API недоступен)
PL_SETTINGS_TRUST_LOCAL_FALLBACK = os.getenv(
    "CONTENTHUB_PL_SETTINGS_TRUST_LOCAL_FALLBACK", ""
).lower() in ("1", "true", "yes")
# То же для advertisers.json и любых сценариев «только локальный PreLend» (алиас над SETTINGS)
PL_TRUST_LOCAL_FALLBACK = os.getenv(
    "CONTENTHUB_PL_TRUST_LOCAL_FALLBACK", ""
).lower() in ("1", "true", "yes")
